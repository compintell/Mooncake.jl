@testset "tangents" begin
    ...
    __x = randn(10)
    p = pointer(__x, 3)
    @testset "set_immutable_to_zero($(Phi._typeof(x)))" for x in Any[
        NoTangent(),
        5.0,
        5f0,
        (5.0, NoTangent()),
        ...
        @test Phi.set_immutable_to_zero(x) isa Phi._typeof(x)
    end

    # Bulk test auto-generated tangents.
    @testset "autogenerated $n $x" for (n, x) in collect(enumerate(Phi.TestTypes.PRIMALS))
        (interface_only, p) = x
