@testset "tangents" begin
    ...
    @testset "autogenerated $n $x" for (n, x) in collect(enumerate(Taped.TestTypes.PRIMALS))
        (interface_only, p) = x
        TestUtils.test_tangent_consistency(sr(1), p; interface_only)
        TestUtils.test_tangent_performance(sr(1), p)
    end
