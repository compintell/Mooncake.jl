@testset "ir_normalisation" begin
    @testset "foreigncall_to_call" begin
        ...
        call = Phi.foreigncall_to_call(foreigncall, sp_map)
        @test Meta.isexpr(call, :call)
        @test call.args[1] == Phi._foreigncall_
    end
    @testset "new_to_call" begin
        new_ex = Expr(:new, GlobalRef(Phi, :Foo), SSAValue(1), :hi)
        call_ex = Phi.new_to_call(new_ex)
        @test Meta.isexpr(call_ex, :call)
        @test call_ex.args[1] == Phi._new_
    ...
    @testset "intrinsic_to_function" begin
        @testset "GlobalRef" begin
            ...
            wrapper_ex = Phi.intrinsic_to_function(intrinsic_ex)
            @test wrapper_ex.args[1] == Phi.IntrinsicsWrappers.abs_float
        end
        ...
        @testset "cglobal" begin
            ...
            wrapper_ex = Phi.intrinsic_to_function(cglobal_ex)
            @test wrapper_ex.args[1] == Phi.IntrinsicsWrappers.__cglobal
        end
    end
    @testset "lift_getfield_and_others $ex" for (ex, target) in Any[
        ...
        @test Phi.lift_getfield_and_others(ex) == target
    end
end
