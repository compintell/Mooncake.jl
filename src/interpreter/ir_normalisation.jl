...
It is possible for element of the `values` field of a `PhiNode` to be undefined. In such
situations, it must be guaranteed that the SSA to which the `PhiNode` writes is not used.
In this situation, we set any undefined values to the first assigned element of the
`values` field.
"""
function rebind_phi_nodes!(ir::IRCode)
    for block in ir.cfg.blocks
        ...
        insts = ir.stmts.inst[block.stmts]
        n = findlast(x -> x isa CC.PhiNode, insts)

        # If this basic block has no PhiNodes, continue to the next basic block.
        n === nothing && continue

        # Replace any undefined fields in each phi node with an arbitrary value. This is
        # safe because any undefined value is guaranteed never to be used.
        for j in 1:n
            node = insts[j]
            i = findfirst(i -> isassigned(node.values, i), eachindex(node.values))
            dummy_value = node.values[i]
            foreach(eachindex(node.values)) do k
                if !isassigned(node.values, k)
                    node.values[k] = dummy_value
                end
            end
        end
