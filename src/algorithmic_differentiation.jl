...
isprimitive(::RMADContext, ::typeof(Umlaut.__splatnew__), T, x...) = true 
...
function taped_rrule!!(::Dual{typeof(Core.typeassert)}, x, T)
    function typeassert_pullback!!(ȳ, ::NoTangent, x̄, ::NoTangent)
...
function run_reverse_pass!(ctx::RMADContext, tape::Umlaut.Tape)
    ...
    for n in reverse(eachindex(ctx.pbs))
        ...
        display(cotangent.(getfield.(ctx.pbs, :yȳ)))
        println()
    end
    return nothing
end
