...
function build_rrule(
    ...
    try
        ...
        if haskey(interp.oc_cache, oc_cache_key)
            ...
            comms_insts = create_comms_insts!(ad_stmts_blocks, info)
            shared_data = shared_data_tuple(info.shared_data_pairs)
            Tshared_data = _typeof(shared_data)

            fwds_ir = forwards_pass_ir(
                primal_ir, ad_stmts_blocks, comms_insts, info, Tshared_data
            )
            pb_ir = pullback_ir(
                primal_ir, Treturn, ad_stmts_blocks, comms_insts, info, Tshared_data
            )
...
@inline push_comms_stack!(comms_stack::SingletonStack, _) = comms_stack

@inline function pop_comms_stack!(comms_stack_ref::Ref)
    item, new_stack = pop!(comms_stack_ref[])
    comms_stack_ref[] = new_stack
    return item
end

@inline pop_comms_stack!(comms_stack::SingletonStack) = pop!(comms_stack)
...
1. create a reference to a `Stack` whose elements are `Tuple`s that can contain all of the
    values associated to the `comms_id`s for each statment. Put this stack in shared data.
2. create instructions which should be inserted at the header block of the forwards-pass and
    reverrse-pass to retreive this reference from shared data, and put it in another
    reference.
3. create instructions which should be inserted in the footer block of the forwards- and
    reverse-passes which transfer the `Stack` from the reference created in 2 back into
    shared data.
4. create instructions which should be inserted at the _end_ of the block generated to
    perform the forwards-pass (in `forwards_pass_ir`) which will put all of the data
    associated to the `comms_id`s into shared data, and
5. create instruction which can be inserted at the _start_ of the block generated to perform
...
```julia
@NamedTuple{
    header_insts::Vector{IDInstPair}, # item 2
    footer_insts::Vector{IDInstPair}, # item 3
    forwards_insts::Vector{Vector{IDInstPair}}, # item 4
    adjoint_insts::Vector{Vector{IDInstPair}}, # item 5
}
```

The nth element of the `forwards_insts` and `adjoint_insts` `Vector`s
correspond to the instructions to be inserted into the forwards- and reverse
...
function create_comms_insts!(ad_stmts_blocks::ADStmts, info::ADInfo)
    insts = map(ad_stmts_blocks) do (_, ad_stmts)
        ...
        stack = Base.issingletontype(TT) ? SingletonStack{TT}() : Ref(ImmutableStack{TT}())
        ...
        # Create instruction for footers.
        footer_inst = (
            ID(), new_inst(Expr(:call, set_stack_ref!, comms_stack_id, local_ref_id))
        )
        ...
        return header_inst, footer_inst, fwds_insts, rvs_insts
    end
    return (
        header_insts=map(x -> x[1], insts)::Vector{IDInstPair},
        footer_insts=map(x -> x[2], insts)::Vector{IDInstPair},
        forwards_insts=map(x -> x[3], insts)::Vector{Vector{IDInstPair}},
...
#=
    forwards_pass_ir(
        ir::BBCode, ad_stmts_blocks::ADStmts, comms_insts, info::ADInfo, Tshared_data
    )
...
function forwards_pass_ir(
    ...
    # Push the header id onto the block stack if needed. Create `LazyZeroRData` for each
    ...
    header_stmts = vcat(
        sds,
        comms_insts.header_insts,
        block_stack_ref_stmt,
        lazy_zero_rdata_insts,
        push_block_stack_insts,
    )
    header_block = BBlock(info.entry_id, header_stmts)
    ...
    # 4. return the BBlock.
    fwds_comms_insts = comms_insts.forwards_insts
    blocks = map(ad_stmts_blocks, fwds_comms_insts) do (block_id, ad_stmts), _comms_insts
        ...
        for stack_inst in _comms_insts
    ...
    # Construct footer block.
    footer_block = BBlock(
        info.exit_id,
        [return_phi_stmt, block_stack_ref_stmt, comms_insts.footer_insts..., return_stmt],
    )

    # Create and return the `BBCode` for the forwards-pass.
    arg_types = vcat(Tshared_data, map(fcodual_type ∘ _type, ir.argtypes))
    blocks = vcat(header_block, blocks, footer_block)
...
#=
    pullback_ir(
        ir::BBCode, Tret, ad_stmts_blocks::ADStmts, comms_insts, info::ADInfo, Tshared_data
    )
...
function pullback_ir(
    ...
    header_block = BBlock(ID(), header_stmts)
    ...
    main_blocks = map(
        ad_stmts_blocks, enumerate(ir.blocks), comms_insts.adjoint_insts
    ...
    # Create a footer block. Dereferences reverse-data for arguments, increments a zero
    # rdata against it to ensure that it is of the correct type, and returns it.
